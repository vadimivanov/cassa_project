...
RAD.namespace('RAD.Blanks.ScrollableRefreshableView', RAD.Blanks.View.extend({
    ...
    attachScroll: function () {
        ...
        self.mScroll = new window.iScroll(scrollView, {
            ...
            onRefresh: function () {
                pullUpEl = self.el.querySelector('#pullUp')
            },
            onScrollMove: function () {

                if (this.y < (this.maxScrollY - 5) && pullUpEl.className.match('visible') && !(pullUpEl.className.match('visible loading'))) {
                    pullUpEl.className = 'visible flip';
                } else if (this.y > (this.maxScrollY + 5) && pullUpEl.className.match('visible flip')) {
                    pullUpEl.className = 'visible';
                }
            },
            onScrollEnd: function () {
                if (pullUpEl.className.match('visible flip')) {
                    pullUpEl.className = 'visible loading';
                    self.updateModel();
                }
            }
        });
